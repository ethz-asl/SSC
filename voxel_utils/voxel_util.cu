...
__global__ void depth2Grid(double* cam_info, double* vox_info,
  ...
  point_base[0] = cam_pose[0 * 4 + 0] * point_cam[0] +
                  cam_pose[0 * 4 + 1] * point_cam[1] +
                  cam_pose[0 * 4 + 2] * point_cam[2];
  point_base[1] = cam_pose[1 * 4 + 0] * point_cam[0] +
                  cam_pose[1 * 4 + 1] * point_cam[1] +
                  cam_pose[1 * 4 + 2] * point_cam[2];
  point_base[2] = cam_pose[2 * 4 + 0] * point_cam[0] +
                  cam_pose[2 * 4 + 1] * point_cam[1] +
  ...
  // mark vox_binary_GPU
  if (x >= 0 && x < vox_size[0] && y >= 0 && y < vox_size[1] && z >= 0 &&
      z < vox_size[2]) {
...
__global__ void calculate_occupancy_prob(double* cam_info, double* vox_info,
                                         double* depth_data,
                                         double* vox_log_GPU) {
  // Get camera information
  int frame_width = cam_info[0];
  ...
  if (x >= 0 && x < vox_size[0] && y >= 0 && y < vox_size[1] && z >= 0 &&
    ...
    int vox_idx = z * vox_size[0] * vox_size[1] + y * vox_size[0] + x;
    vox_log_GPU[vox_idx] =
        min(vox_log_GPU[vox_idx] + log(prob_occ / (1 - prob_occ)),
            log(max_prob / (1 - max_prob)));
  }
}

__global__ void tsdfTransform(double* vox_info, double* vox_tsdf) {
...
__global__ void SquaredDistanceTransform(double* cam_info, double* vox_info,
                                         double* depth_data,
  ...
  point_base[1] = point_base[1] - cam_pose[1 * 4 + 3];
  point_base[2] = point_base[2] - cam_pose[2 * 4 + 3];
  point_cam[0] = cam_pose[0 * 4 + 0] * point_base[0] +
                 cam_pose[1 * 4 + 0] * point_base[1] +
                 cam_pose[2 * 4 + 0] * point_base[2];
  point_cam[1] = cam_pose[0 * 4 + 1] * point_base[0] +
                 cam_pose[1 * 4 + 1] * point_base[1] +
                 cam_pose[2 * 4 + 1] * point_base[2];
  point_cam[2] = cam_pose[0 * 4 + 2] * point_base[0] +
                 cam_pose[1 * 4 + 2] * point_base[1] +
  ...
  if (pixel_x < 0 || pixel_x >= frame_width || pixel_y < 0 ||
      pixel_y >= frame_height) {  // outside FOV
    return;
  }
  ...
  for (int iix = max(0, x - search_region);
       iix < min((int)vox_size[0], x + search_region + 1); iix++) {
    for (int iiy = max(0, y - search_region);
         iiy < min((int)vox_size[1], y + search_region + 1); iiy++) {
      for (int iiz = max(0, z - search_region);
           iiz < min((int)vox_size[2], z + search_region + 1); iiz++) {
        ...
        if (vox_binary_GPU[iidx] > 0) {
          ...
          double zd = abs(z - iiz);
          double tsdf_value =
              sqrtf(xd * xd + yd * yd + zd * zd) / (double)search_region;
...
void ComputeTSDF(double* cam_info_CPU, double* vox_info_CPU,
  ...
  cudaMemcpy(cam_info_GPU, cam_info_CPU, 27 * sizeof(double),
             cudaMemcpyHostToDevice);

  // copy vox info to gpu
  double* vox_info_GPU;
  cudaMalloc(&vox_info_GPU, 8 * sizeof(double));
  cudaMemcpy(vox_info_GPU, vox_info_CPU, 8 * sizeof(double),
             cudaMemcpyHostToDevice);

  // copy depth data to gpu
  double* depth_data_GPU;
  cudaMalloc(&depth_data_GPU, frame_height * frame_width * sizeof(double));
  cudaMemcpy(depth_data_GPU, depth_data_CPU,
             frame_height * frame_width * sizeof(double),
             cudaMemcpyHostToDevice);

  // copy depth mapping to gpu
  double* depth_mapping_idxs_GPU;
  cudaMalloc(&depth_mapping_idxs_GPU,
             frame_height * frame_width * sizeof(double));
  cudaMemcpy(depth_mapping_idxs_GPU, depth_mapping_idxs_CPU,
             frame_height * frame_width * sizeof(double),
             cudaMemcpyHostToDevice);
  ...
  cudaMemcpy(vox_tsdf_GPU, vox_tsdf_CPU, num_crop_voxels * sizeof(double),
             cudaMemcpyHostToDevice);

  // from depth map to binaray voxel representation
  depth2Grid<<<frame_width, frame_height>>>(cam_info_GPU, vox_info_GPU,
                                            depth_data_GPU, vox_binary_GPU,
                                            depth_mapping_idxs_GPU);
  ...
  cudaMemcpy(vox_tsdf_CPU, vox_tsdf_GPU, num_crop_voxels * sizeof(double),
             cudaMemcpyDeviceToHost);
  cudaMemcpy(depth_mapping_idxs_CPU, depth_mapping_idxs_GPU,
             frame_height * frame_width * sizeof(double),
             cudaMemcpyDeviceToHost);
  cudaMemcpy(occupancy, vox_binary_GPU, num_crop_voxels * sizeof(double),
             cudaMemcpyDeviceToHost);
...
void calculateOccupancyProb(double* cam_info_CPU, double* vox_info_CPU,
  ...
  cudaMemcpy(vox_prob_GPU, vox_prob_CPU, num_crop_voxels * sizeof(double),
             cudaMemcpyHostToDevice);
  // GPU_set_zeros(num_crop_voxels, vox_binary_GPU);
  ...
  calculate_occupancy_prob<<<frame_width, frame_height>>>(
      cam_info_GPU, vox_info_GPU, depth_data_GPU, vox_prob_GPU);

  // copy computed log odds back to CPU
  cudaMemcpy(log_odds_occupancy, vox_prob_GPU, num_crop_voxels * sizeof(double),
             cudaMemcpyDeviceToHost);
